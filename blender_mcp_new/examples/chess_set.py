...
def create_chess_set():
    ...
    try:
        ...
        # 验证棋盘是否真的存在
        verify_response = client.verify_object_exists(board_name)
        if not verify_response.get("status") == "success" or not verify_response.get("result", {}).get("exists", False):
        ...
        # 添加日光以提供整体照明
        sun_response = safe_operation(client, client.create_object, "LIGHT", name="Sun", 
                                    location=[5, 5, 10],
                                    operation_name="创建太阳光")
        sun_name = get_object_name(sun_response) or "Sun"
        debug_print(f"使用的阳光名称: {sun_name}")
        
        # 设置灯光类型和能量
        safe_operation(client, client.set_light_type, sun_name, "SUN",
                      operation_name="设置太阳光类型")
        safe_operation(client, client.set_light_energy, sun_name, 3.0,
                      operation_name="设置太阳光强度")
        
        # 添加高级区域光照为场景增添气氛
        debug_print("创建区域光照...")
        area_response = safe_operation(client, client.create_advanced_lighting, 
                                    name="Chess_Light",
                                    light_type="AREA",
                                    location=[0, 0, 5],
                                    energy=80,
        ...
        if scene_info and "result" in scene_info and "objects" in scene_info["result"]:
            for obj in scene_info["result"]["objects"]:
                if obj.get("type") == "CAMERA":
                    camera_name = obj.get("name")
                    break
        
        if not camera_name:
            # 创建新相机
            camera_response = safe_operation(client, client.create_object, "CAMERA", name="MainCamera",
                                          location=[8, -8, 6],
                                          operation_name="创建相机")
            camera_name = get_object_name(camera_response) or "MainCamera"
        else:
            # 移动现有相机
        ...
        # 5. 渲染场景
        debug_print("渲染场景...")
        safe_operation(client, client.render_scene, 
                      resolution_x=1920, 
                      resolution_y=1080, 
                      output_path="//chess_render.png",
                      operation_name="渲染场景")
        
        print("国际象棋场景创建完成！")
        
    except Exception as e:
        print(f"创建国际象棋场景时发生错误: {str(e)}")
        traceback.print_exc()
    finally:
        # 确保断开连接
        client.disconnect()
...
def create_pawn(client, name, location, is_white=True):
    ...
    try:
        ...
        # 检查底座是否成功创建
        verify_response = client.verify_object_exists(base_obj_name)
        if not verify_response.get("status") == "success" or not verify_response.get("result", {}).get("exists", False):
        ...
        if merge_response and merge_response.get("status") == "success":
            # 验证合并后的对象是否存在
            verify_response = client.verify_object_exists(base_obj_name)
            if verify_response.get("status") == "success" and verify_response.get("result", {}).get("exists", False):
