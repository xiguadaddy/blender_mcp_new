...
logger = get_logger("BlenderMCP.MeshTools")

# 记录加载日志
logger.info("正在加载mesh_tools包")
...
for file_path in Path(current_dir).glob("*.py"):
    ...
    if filename != "__init__.py" and filename.endswith(".py"):
        ...
        try:
            # 动态导入模块
            module = importlib.import_module(f"..mesh_tools.{module_name}", __package__)
